...
const Hero = () => {
  ...
  return (
    <>
      <Navbar />
      <Menu />
      <section className="flex gap-6 min-h-full sm:h-[90vh]  justify-center items-center flex-row py-[3rem] flex-wrap md:flex-nowrap">
        <div className="font-body text-charcoal text-center sm:text-left space-y-3 w-full sm:w-[60%]">
          ...
          <button className="bg-beige/70 transition-all duration-300 ease-linear hover:bg-beige rounded-lg px-6 py-3">
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-0 w-full sm:w-[40%]">
          {collage.map((photo) => (
            <div key={photo.id} className="w-[100%] h-[200px] rounded-tl">
              <img
                className={photo.roundedPosition}
                src={photo.src}
                alt={photo.alt}
              />
            </div>
